...
const ensureYouTubeIframeApi = () => {
  ...
  youtubeApiReadyPromise = new Promise((resolve, reject) => {
    ...
    const existingScript =
      document.getElementById(YT_API_SCRIPT_ID) ||
...
export const useYouTubePlayer = ({
  ...
  const playWithAutoplayFallback = useCallback(() => {
    ...
    // Retry muted only when initial autoplay is blocked.
    window.setTimeout(() => {
      if (!playerRef.current || !readyRef.current) return
  ...
  const syncPlay = useCallback((time) => {
    if (!playerRef.current || !readyRef.current) return
  ...
  const syncPause = useCallback((time) => {
    if (!playerRef.current || !readyRef.current) return
    suppressRef.current = true
    playerRef.current.seekTo(time, true)
  ...
  const setCaptionsEnabled = useCallback((enabled, languageCode = 'en') => {
    if (!playerRef.current || !readyRef.current) return
    ...
    try {
      ...
      // Some videos may not expose captions controls.
    }
  }, [])
