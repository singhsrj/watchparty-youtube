...
const ParticipantsPanel = ({ participants, me }) => {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-4 py-3 border-b border-border">
        <h3 className="font-display font-semibold text-sm text-white">Participants</h3>
        <span className="font-mono text-xs text-slate-500 bg-surface px-2 py-0.5 rounded-full">{participants.length}</span>
      </div>
                ...
                <div className="relative">
                  ...
                  {menuOpen && (
                    <div className="absolute right-0 top-7 z-50 w-48 bg-panel border border-border rounded-xl shadow-card overflow-hidden animate-slide-up">
                      <div className="p-1">
                        <p className="px-3 py-1.5 text-xs font-mono text-slate-500 uppercase tracking-wide">Assign Role</p>
                        {['moderator', 'participant'].map((role) => (
                          <button
                            key={role}
                            onClick={() => handleAssignRole(p.socketId, role)}
                            disabled={p.role === role}
                            className="w-full text-left px-3 py-2 text-sm font-body text-slate-300 hover:bg-white/5 rounded-lg transition-colors disabled:opacity-30"
                          >
                            <span
                        ...
                        <button
                          ...
                          className="w-full text-left px-3 py-2 text-sm font-body text-amber-400 hover:bg-amber-400/5 rounded-lg transition-colors flex items-center gap-2"
                        >
                          <ArrowUpDown size={13} />
                          Transfer Host
                        </button>
                        <button
                          ...
                          className="w-full text-left px-3 py-2 text-sm font-body text-danger hover:bg-danger/5 rounded-lg transition-colors flex items-center gap-2"
                        >
                          <X size={13} />
                          Remove
                        </button>
